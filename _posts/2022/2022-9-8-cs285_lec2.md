...
layout: post
Created: September 5, 2022 11:02 AM
tags:
    - CS285
use_math: true
comments: true
...
Now we know from SL theory, when you train a particular model on a particular train distribution and you get good training error, and don’t overfit, you can expect to also get good test eror if test points are drawn from same distribution.
...
**After a while, $p_{\pi_{\theta}}(o_t)$ is different from $p_{data}(o_t)$. So this is reason fro compunding error problem.**

So can we make $**p_{\pi_{\theta}}(o_t)$ eqaul to $p_{data}(o_t)$? If we can do this, then we know that our policy would produce good action simply from standard results in supervised learning theory.**

Now one way to make $p_{\pi_{\theta}}(o_t)$ eqaul to $p_{data}(o_t)$ is to simply make the policy pefect lol. The policy is pefect then never makes mistakes then distribution will match but very very hard.

---

![Untitled 18](https://i.imgur.com/Y1jXYZn.png)
instead of being clever of policy, we actaully be clever about our data distribution. So not change the policy, but change data to avoid distributional shift problem.

That’s the basic idea behind a method DAgger.

goal: collect trainig data that comes from $p_{\pi_{\theta}}(o_t)$ instead of $p_{data}(o_t)$. Because we have observation, action tuples from $p_{\pi_{\theta}}(o_t)$ and we train on those tuples then distributional shift problem will be gone.
...
Imagine you’re watching a video of drone flying thorugh forest, and you have to steer that drone and provide optimal actions without your actions actullay affecting the drone in real time. (that’s very unnature.)
...
This is not limited to Non-markovian/markovian issue and it can happen in all sorts of imitation learning scenarios. But two question migh ask you to ponder.
...
>
1. Output mixutre of gaussians. (not use single gaussian output distributions.) This can capture mutliple mode.
2. Latent variable model
3. Autoregressive discretiazaiton
...
Last option strikes a good balance between simplicity and expressitvity.
...
- If we have discrete action, this multi-modality problem is not issue. Because of discrete action, a sofmax Categorical distribution can easily represent any distribution.
...
This is why behavior cloning is not actually optimizing quite the right objective. Behavior cloning typically maximize the loglikelihood in expectation under the state or observation under the expert. → whereas we want to maximize in expectation under the state,observation distribution of our policy. → Distributional mismatch problem (DAgger tries to correct)
...
Bounding total number of mistakes, we’re going to most pessimistic scenario possible.  (worst possible case. == tightrope walker scenario. when they fall of to red region, there is no demonstration data in trainig because expert never fall off. they don’t know what to do in red region.)
