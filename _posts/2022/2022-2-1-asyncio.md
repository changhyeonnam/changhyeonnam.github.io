...
실행중에 멈출거나 다시 실행될 수있는 function을 coroutine이라고 한다. Python에서는 `async def` 를 이용하여 이를 정의한다. generator와 많이 유사한데, yield from과 같은 동작을 하는 await 이라는 keyword가 있다. 아래 코드와 같이 `await`은 `yield from`과 동일하게 작성된다.
