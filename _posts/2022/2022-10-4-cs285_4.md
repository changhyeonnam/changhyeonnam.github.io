...
sitemap :
  ...
  priority : 1.0
---
> CS285 Lecture3는 파이토치 관련한 내용이라 생략했다. Lecture 4에서는 MDP가 무엇인지 복습하고, state value function, state-action value function인 Q function에 대해서 다뤘다. 또한 다양한 RL algorithm 종류들에 대해 간단하게 훑고, RL algorithm마다 주요하게 생각하는 trade-off, assumption들을 다뤘다.
>

![Untitled 0](https://i.imgur.com/TrO9n4F.png)
---
...
- $\Tau_{i,j,k}$를  $\mu_{t+1,i}$를 계산하기 위한 linear operator로 볼 수 있다.

![Untitled 5](https://i.imgur.com/e1BmFvL.png)
Partially observed MDP에 대한 내용이다.

- 추가적인 두가지 object이 필요하다. Observation space $O$, emission(observation) probability $\Epsilon$.
...
- MDP에서, trajectory에 대한 prob distribution을 적어 볼 수 있다.
    ...
    - $p_{\theta}(\tau)$ : trajectory를 $\tau$로 표시한다.
...
그래서 이 markov chain을 충분히 forward를 했을때, 결국 $s_t,a_t$에 대한 분포는 $\mu$에 수렴할 것이고, t가 $\infin$으로 간다면, 이 marginal값의 합의 expectation은 stationary distribution $\mu$에 의해 결정될 것이다.
