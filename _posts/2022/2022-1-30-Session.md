...
Session은 또한 request method에 대해 default data를 제공할 때도 사용할 수 있다. Session object에 property를 할당하는 방식으로 구현된다. Session-level로 전달된 dictionary는 request method에 merge된다. method-level로 전달된 parameter는 session parameter에 override된다. method-level로 전달된 것은 session 사용 유무에 상관없이 persist하지 않다. 다음 예시를 보면 확인할 수 있다.
