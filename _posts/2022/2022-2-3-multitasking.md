...
동시성, 병렬성을 활용하면 다양한 프로그램 경로나 다양한 I/O 흐름을 이용할 수 있다. 병렬성과 동시성의 가장 핵심적인 차이는 속도에 있다. 서로 다른 두 프로그램이 병렬적으로 실행되면 실제 전체 작업을 하는데 걸리는 시간이 두배 빨라진다. 동시성을 적용한 프로그램은 겉으로 보이기엔 병렬적으로 실행되는 것처럼 보이지만, 걸리는 시간이 단축되지는 않다.
...
operating system이 아무 시점에서 특정 task로 context switch하기 위해 해당 thread가 선점하게 할 수 있다. 이를 preemptive multitasking이라 한다. threading에서는 해당 task로 switch하기 위해 별도의 코드를 작성하지 않아도 되지만, 어떠한 시점에서도 switch될 수 있기 때문에 예상치 못한 문제가 생길 수 있다.  
...
cpu core개수만큼 process를 병렬적으로 돌릴 수 있다. 만약 core 개수 이상의 process로 multiprocess를 한다면, core 개수의 process만 동작하고 나머지는 ready queue에서 기다린다.
...
1. I/O Bound Program
    ...
    위의 사진과 같은 경우가 I/O Bound한 program이다. 브라우저를 동작시키는 프로세스가 대부분 위와 같이 동작한다. 대부분의 시간을 기다리는데 사용하고, CPU instructions에 대해선 그보다 적은 시간을 사용한다. 이러한 devices 혹은 external resource에 대해 사용하는 시간을 줄이는 것이 I/O Bound한 task를 빨리하는 방법이다.

2. CPU Bound Program

    ![Untitled](/images/2022/python/mt/Untitled1.png)

    I/O Bound와 반대로, network 또는 file을 접근하는 것 없이, 중요한 cpu 연산을 하는 프로그램을 cpu bound 하다고 한다. 대부분의 시간을 CPU operation을 하는 데에 사용한다. 같은 시간동안 더 많은 연산을 하는 것이 CPU Bound한 task를 더 빨리 하는 방법이다.


I/O Bound program에 대해서 동시성에 대한 방법을 차례대로 적용하여 얼마나 시간이 단축되는지 살펴볼 것이다. 또한 CPU Bound program에 대해서도 병렬성을 적용하여 시간이 단축되는지 살펴볼 것이다.
...
`download_site` 는 url 으로 부터 content를 다운로드 받고, 크기를 출력하는 함수이다. 이때, `Session()` object을 이용하여 tcp connection을 persist하게 만들어 조금 더 빠르게 다운받게 하였다. (Session에 대한 내용은 다음 [post](https://changhyeonnam.github.io/2022/01/30/Session.html)를 참고하면 좋을 것 같다.)
...
threading을 할때 고려해야 하는 issue가 한가지 더 있다. os가 실행중인 task를 인터럽트하거나, 새로운 task를 실행할때, thread 간에 공유되는 모든 data는 thread-safe해야 한다. 하지만 requests.Session()은 thread-safe하지 않다. 이를 위해 data를 thread-safe하게 access하는 몇가지 전략이 있다. data의 종류에 따라, 어떻게 그 data를 사용하냐에 따라 전략이 달라진다.

첫번째 전략은 Queue와 같이 thread-safe data structure를 사용하는 것이다. threading.Lock과 같은 low-level을 사용하여 어떤 시점에 대해 하나의 thread만 block of code 또는 bit of memory에 접근하게 하는 방법이다.
...
두번째 전략은 코드에서 사용한 thread local storage를 사용하는 것이다. threading.local()은 global object처럼 보이게 만들지만, 각 thread에 specific한 object 만들어준다. 만들어진 object이 각 thread에 대한 데이터의 접근을 분리시켜준다. `get_session()` 이 호출되었을 때, 해당`session` 이 특정한 thread를 lookup하게 한다. 그래서 각 thread는 single session을 만들어서, 해당 session을 Lifetime동안 사용하게 된다.
...
위에서 언급한바와 같이 `await`을 이용하여 event loop에 control을 다시 돌려줄 수 있다. 예를 들어, 어떤 function call을 `await`한다는 것은 function을 실행 마치고 나서, control을 다시 돌려준다는 것을 의미한다.

`async`는 `await`을 사용하여 function을 정의하겠다는 flag로 생각하면 이해하기 쉽다. 항상 이런방식으로 사용되지는 않는다. 예를 들어 `async with을` 사용하여, await하는 object으로부터 context manger를 만드는 것이다. 하지만 context manager를 flag하는 concept은 동일하다.
