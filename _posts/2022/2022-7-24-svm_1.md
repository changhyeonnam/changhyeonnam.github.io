...
2. 3개의 벡터를 찾았다면, 같은 클래스에 속한 2개의 벡터를 연결하여 하나의 직선을 긋습니다.
3. 2번에서 그은 직선과 평행한 직선 중 나머지 벡터(파란색)을 지나는 직선을 긋습니다.
4. 2번과 3번에서 그은 직선들의 중점을 지나는 평행한 직선(초록색)을 긋습니다. 이 초록색 직선이 서포트 벡터 머신의 결정 경계 입니다.


여기 마진은 결정경계에서 가장 가까운 점에서 수선의 발을 내렸을 때의 거리(Perpendicular distance)을 의미합니다. 위의 사진에서, 양쪽 점선에서 실선까지의 거리를 뜻합니다.
...
$$ argmin\mid\mid w \mid\mid = \sqrt{w_1^2+w_2^2+..+w_i^2} $$
