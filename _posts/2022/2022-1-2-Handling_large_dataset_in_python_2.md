...
위의 두가지 method는 pickling process에 사용하는 method로써, 주어진 obj에 대해 seriailize합니다. `dump()` 와 `dumps()` 의 차이는 첫번째 method는 serialization result를 포함하는 file을 생성하고, serialization result를 byte object를 반환합니다.
...
pickling process를 거친 obj혹은 bytes열은 python에서만 사용할 수 있다. 하지만 python의 버전에 따라 support하는 pickle protocol이 달라진다. 즉, python3.8까지 support하는 protocol version5에서의 pickled obj을  python 3.0을 supprot하는 protocol 3의 pickle 모듈의 load로 unpickle할 수 없다는 것을 의미한다.

많은 object type을 pickling할 수 있지만, pickable하지 않은 타입도 있다. 대표적인 unpickable한 타입들은 database connections, opened network sockets, running threads을 포함한 object들이다. `dill` 이라는 모듈은 pickle의 확장버전으로  이를 사용하면 function with yield, nested fucntion에 대해서도 pickling이 가능하다.
