...
### tmi
논문을 발표하면서, 들어온 질문이 그러면 해당 모델은 단점이 없나요? 라는 질문이었습니다. 이 질문이 요즘 제가 논문을 읽는 태도를 돌이켜 보게 하였습니다. 비판적으로 읽기 보다는, 읽고 구현된 모델 돌려보고 에이 안되네 이러거나 단점을 생각할때도, 단순하게 이런데서 느리고, 이런걸 못하지 않을까 정도지 깊게 생각은 안해 보았었습니다. 지금 생각해보면 RNN 계열의 근본적인 단점인, 마지막 layer(cell)이 모든 정보를 표현해야 하기 때문에 그로인한 정보 손실이 해결되지 않았을것 같습니다. 그리고 정말 큰 모델을 만든다면 해당 논문에서 제시한 SRU++ layer의 어떤부분에 attention을 쓰고, 몇개를 쓸지 모두 휴리스틱하게 실험을 하지 못할 것 같습니다. (아니면 말고 ㅎㅎ)
