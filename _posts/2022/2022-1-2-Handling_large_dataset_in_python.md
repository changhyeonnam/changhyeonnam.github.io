...
사실 최근에 코드를 잘때, iterrows()와 같이 dataframe을 loop을 돌면서 전처리 하는 코드를 짰다. 이번에 공부하면서 최대한 vectoration연산을 하는 method를 활용해서 다시 작성하엿다. jupyter notebook에서 돌렸을때 작업 속도가 체감될 정도로 차이가 났다.
