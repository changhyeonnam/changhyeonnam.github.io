...
1. understand how object is rigid, soft, heavy, and desing soultion.
2. set tuple of (image, coordinate), and train with supervised learning.

![Untitled 1](https://i.imgur.com/NVMvlnw.png)

RL can be used with  good sucessful behavior data instead of Ground truth data. (In ohter words, we need to tell what experience is possible outcome in real-world.

---

### What is Reinforcement learning?

1. Mathematical formalism for learning-based decsion making.
2. Approach for learning decsion making and control from experience.
...
In short, we are using deep learning to hadle flexibility,complexity, unpredicitablity of real-world.

![Untitled 3](https://i.imgur.com/i6UBMCw.png)
In open world setting, you need to model that generalize effienctly to un seen data.
...
previously, we should manually desinged feature but by using deep learinng, we can automatically learned features.
...
summary) Deep models are waht allow RL algorithm to solve complex problem end to ends.

---

![Untitled 8](https://i.imgur.com/GcSojg3.png)
- Basic RL assume that it can interact  with system and receive ground truth reawrd.
- Learning to predict and using prediction to act → instead of learning behavior directly, whay you attempt to learn is some representation of how the world works. (= model  based RL)
...
- More than Imitation : Infering intentions → infer the goal to find more effective way using RL.
- Prediction for real-world control (=domain of model-based RL) : not learn a policy but learn a model that predict what will happen next in given obseravation.
...
> Instead of trying to produce a program to simulate hte adult mind, why not rather try to produce one which simulates the child’s? If this were then subjected to an appropriate course of education one would obtain the adult brain.
>
