...
- $(x_i,x_i^+)$ : semantically related pair
- $D=\{(x_i,x^+_i)\}^m_i$ : set of related pair.
- $l_i = -log({{e^{sim(h_i,h_i^+)}/\tau}\over{\sum^N_{j=1}e^{sim(h_i,h^+_j)}/\tau}})$ : (cross-entropy) training objective for $(x_i,x_i^+)$  with mini-batch of N pairs.  $\tau$ is a temperature hyperparameter
- $sim(h_1,h2)$ : cosine similarity $(h_1,h_2)$
...
$l_i = -log({{e^{sim(h^{z_i}_i,h_i^{z'_i}}/\tau}\over{\sum^N_{j=1}e^{sim(h_i^{z_i},h^{z'_j}_j)}/\tau}})$  : for mini-batch of N sentences.
...
![Untitled 9](https://i.imgur.com/1tpHexL.png)
