...
작년에 naver ai rush 이후, nlp에 빠져서 한동안 공부했다. NLP와 관련되어, 관심이 생겼던 분야는 특정 task가 아닌, large scale model 혹은 foundation model을 학습하여 더 효율적이고, 경량화된 모델을 만드는 것에 관심이 있었다. 하지만 공부를 하면 할수록 정말 많은 리소스가 있는 회사에서만 큰 모델에 대해 시도를 해볼 수 있고, 학생의 입장에서 이것저것 시도하기엔 한계가 많았다. (단순 inference가 아니라면?, 사실 할라면 할 수는 있었다고 생각함.)
