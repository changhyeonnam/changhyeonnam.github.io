...
A번 문제는 구간 내에서 가장 큰 두 값을 찾는 것입니다. 세그먼트 트리의 각 노드에 최대값과  두번째 최대값을 둘다 저장하면 됩니다.  
...
- merge
    ...
    a와 b 노드 에 해당하는 구간에서 최대값과 두번째 최대값을 한개의 노드에 합쳐서 반환하는 함수 입니다.
...
직사각형의 세로 변을 정할 때 금광의 좌표 들중에 가능한 모든 [y1,y2]쌍에 결정하는 데 $O(N^2)$의 시간이 걸립니다. 만약 [y1,y2]가 고정되었다고 해봅시다.

그러면 주어지는 구간의 부분합의 최대값을 구하는 세그먼트 트리를 이용하여 $O(N^2*logN)$시간에 구현이 가능합니다.

다음은 구간의 부분합의 최대값을 구하기 위해 분할정복을 사용한 설명입니다.
...
merge한 큰 구간의 부분합의 최대값은 구간1에만 존재할 수도 있고, 구간2에만 존재할수도 있고, 구간1과 구간2를 모두 걸쳐서 존재 가능합니다. 앞선 2가지는 이미 계산이 되어 있고, 걸쳐서 존재하는 경우에 대해서 계산해줘야 합니다.  이것은 왼쪽 구간에서 오른쪽 끝 원소를 포함하는 부분합의 최대값과 오른쪽 구간에서 왼쪽 끝 원소를 포함하는 부분합의 최대값의 합을 통해 계산할 수 있습니다.
