...
다음 단어를 예측하기 때문에 decoder 자체가 LM이고, encoding된 source sentence에 conditional 합니다. SMT는 에서는 $P(y\mid x)$를 directly learn하지 않고, alignment를 이용한 $P(x,a|y)P(y)$ 같이 break down하여 학습했습니다. NMT는 $P(y\mid x)$ 자체를 직접 학습합니다.
