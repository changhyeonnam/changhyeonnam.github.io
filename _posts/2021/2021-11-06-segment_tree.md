...
구간 합 구하기 문제를 통해 세그먼트 트리의 원리를 설명하겠습니다. 주어진 배열의 원소들이 고정되어 있다면, dp혹은 누적합을 이용하여 구간합을 구할 수 있습니다. 하지만 배열의 원소가 갱신된다면, O(N * M) 은 시간초과가 나기 때문에 세그먼트 트리를 통해 O(logN * M) 의 시간복잡도로 풀어야 합니다. (1≤ N≤10^6 (N=원소의 개수), 1≤M,K≤10^4 (M= 업데이트 횟수,))
