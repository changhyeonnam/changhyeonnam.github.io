...
![Untitled](/images/2021/segment/l.png)
...
구간 함 구하기 문제를 통해 세그먼트 트리의 원리를 설명하겠습니다. 주어진 배열의 원소들이 고정되어 있다면, dp혹은 누적합을 이용하여 구간합을 구할 수 있습니다. 하지만 배열의 원소가 갱신된다면, O(N*M) 은 시간초과가 나기 때문에 세그먼트 트리를 통해 O(logN*M) 의 시간복잡도로 풀어야 합니다. (1≤ N≤10^6 (N=원소의 개수), 1≤M,K≤10^4 (M= 업데이트 횟수,))
...
구간 합 구하기와 구간 합 구하기2의 차이점은 전자는 값을 갱신하는 부분이 i번째에서 일어났다면, 후자는 i번째부터 j번째까지의 구간이 갱신됩니다. 만약 느리게 전파하지 않는 세그먼트로만 구현한다면, O($10^{10})$으로 시간 초과가 납니다.
...
그래서 위와 같이 갱신해야 하는 노드까지만 표시를 해주고 (구간에 해당되는 노드들), lazy prapagation에 대한 트리를 따로 만들어서, 해당 원소 값을 직접 사용해야할때 계산해서 처리해줍니다. 이런식으로 구간 갱신에도 $O(logN*M)$의 시간복잡도를 갖게 됩니다.

현재 위의 트리라면 lz[11]~lz[14]에는 3이 들어가 있습니다. 만약에 [3,3] (11번 노드), [4,4] (12번 노드)에 대해서만 합을 구한다면 해당 노드들의 값이 업데이트가 되고, lz[11],lz[12]는 0으로 초기화가 됩니다.
