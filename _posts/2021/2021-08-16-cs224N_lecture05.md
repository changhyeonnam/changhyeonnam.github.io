...
Phrase structure의 기본적인 idea는 senetence들은 기본적인 단어들이 점진적으로 중첩되어 더 큰 단위로 만들어진다는 것 입니다. 예를들어 위와 같이 words로 시작하여 word들을 합쳐서 phrase를 만들고, phrase들을 합쳐서 bigger phrase로 만드는 것입니다.  
...
Phrase struture에 대한 tree와 그에 해당하는 rule입니다.
...
사람들은 복잡한 의미를 갖는 복잡한 아이디어를 여러 단어를 큰 단위로 구성하며 의사소통 합니다. 그래서 모델이 언어를 올바르게 해석하기 위해서 문장 구조(sentence structure)를 이해해야할 필요가 있습니다.

---

지금부터 문장 구조(sentence structure)를 올바르게 이해하지 못할때 생기는 중의적인 잘못된 해석에 대한 예시를 보겠습니다.
