...
추가로, 모든 그래프 dp문제는 DAG이고, 만약 사이클이 있다면 memorization또한 순환적으로 의존하게 되므로 문제가 생깁니다. 또한 위상정렬이 가능하다는 말은 그래프가 DAG라는 말과 동치입니다.

위상정렬의 구현에는 크게 dfs, bfs로 구현하는 두가지 방법이 있습니다. 각 구현의 차이점은 dfs에서는 out-degree가 0인 것 부터, bfs에서는 in-degree가 0인 것부터 정렬하는 차이점이 있습니다.
...
스폐셜 저지 문제를 처음 풀어봤는데, 답이 여러개가 가능한 경우 스폐셜 저지 문제라고 합니다. (같은 위상인 경우 순서가 없기 때문) dfs로 구현하여 풀었습니다.
...
문제에서 다이나믹한 게임 이라고 나와있어, dp문제인것을 의심할 수 있습니다.(ㅋㅋ) 즉, 그래프가 DAG일수도 있습니다. 그리고 문제를 읽어보면 DAG인 것을 알수 있습니다.

방문할 후행 노드의 시간은 max(선행노드가 완료될 시간+후행노드의 시간, 후행 노드의 시간)으로 구현이 가능합니다.
