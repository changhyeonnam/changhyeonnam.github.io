...
DAG에서 위상에 따라 방향성을 거스르지 않게 나열하는 알고리즘을 위상정렬(topological sort)이라고 합니다. 예를들어 아래 그림에서 위상에 따라 출력해보면, 1,2,4,5,3,6 입니다. (답이 여러개일 수 있습니다.)
...
간단한 위상정렬 문제입니다.문제에서 DAG라고 주어지지는 않았지만, 예를들어 세명의 학생이 있다고 해보자.  첫번째 학생의 키>두번째 학생의 키, 두번째 학생의키 > 세번째 학생의 키라고 했을때, 세번째 학생의 키> 첫번째 학생의 키는 불가능 하므로 DAG인것을 알 수 있습니다.
...
방문하면서 각 정점마다 들어오는 간선의 수(in-degree)를 감소시켜, 0이 아닐 경우에만 dfs를 호출했습니다. 0인 경우, 더 이상 해당 노드의 후행노드 값들에 대해 업데이트하지 않아도 되기 때문입니다.

또한. 어떤 정점에서 시작할지 정해져 있지 않기 때문에, in-degree가 0인 정점들을 큐에 push해주고 큐의 각 정점에 대해 탐색했습니다. 승리하기 위해 건설해야할 건물 번호에 대해 탐색이 되었을 때, 표시를 하여 답을 출력했습니다.
