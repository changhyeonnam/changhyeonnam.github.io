...
> 2017년에 발표된 [Neural Collaborative Filtering](https://arxiv.org/pdf/1708.05031.pdf) 논문을 review한 내용입니다. Matrix  Factorization의 한계를 다룬 내용을 제외한 것은 생략하겠습니다. (다음 [포스트](https://changhyeonnam.github.io/2021/12/21/Matrix_Factorization.html)를 참고하면 됩니다.)
>
...
$M$ : number of users , $N$ : number of  items 이라고 하자.

user의  implicit  feedback으로 user-item interaction matrix $Y ∈ R^{M×N}$  을 정의할것이다.  이때  matrix $Y$의 각 원소 $y_{ui}$는 다음과 같다.
...
NCF framework에서는 pointwise, pairwise learning을 모두 support한다.

### 2.2 Matrix Factorization의 한계

![Untitled](/images/2021/Rec_sys/ncf/ncf0.png)
...
![Untitled](/images/2021/Rec_sys/ncf/ncf1.png)
...
다른방법으로, Bayesian Personalized Ranking, margin-based loss와 같은 pairwise  learning으로도 학습 할수 있다. 이 논문은 neural network modeling part에 초점을 맞추기때문에, pairwise learning은 future work로 미룰것이다.  

이제 NCF’s predictive model를 다음과 같이 formulate할 수 있다.

$\hat y_{ui} = f(P^Tv_u^U,Q^Tv_i^T\mid P,Q,\theta_f)$

$P ∈ R^{M×K} and \space Q ∈ R^{N×K}$ 	 : 각각 user와 item을 위한 latent factor을 의미한다.

$\theta_f$ : function f의  parameter of interaction을 의미한다.

function f가 multi-layer neural network로써 정의되기 때문에 다음과 같이 formulate 될 수 있다.

$f(P^Tv_u^U,Q^Tv_i^I) = \phi_{out}(\phi_X(...\phi_2(\phi_1(P^Tv_u^U,Q^Tv_i^I))...))$, $\phi_{out},\phi_{x}$는 output layer와 x-th ncf layer의 mapping function을 의미한다.
...
![Untitled](/images/2021/Rec_sys/ncf/ncf2.png)
...
GMF, MLP 모델을 학습시킬때 Adam  optimizer가 SGD보다 더 빨리 수렴하여  Adam을 사용하였다.
...
![Untitled](/images/2021/Rec_sys/ncf/ncf3.png)
...
hyper parameter를 결정하기 위해 validation set은 randomly sample하여 사용했고, validation set에 대해 hyperparameter를 이용하여 tune했다. 모든 NCF 모델은 log loss를 optimize하는 것을 학습된다. model parameter들은 Gaussian distribution (with a mean of 0 and standard deviation of 0.01)으로 random하게 초기화 되고, mini-batch Adam으로 optimize된다. 실험한 하이퍼 파라미터는 다음과 같다.
...
![Untitled](/images/2021/Rec_sys/ncf/ncf4.png)
결론만 말하자면, NeuMF가 다른 모든 Baseline들보다 성능이 좋았다.

![Untitled](/images/2021/Rec_sys/ncf/ncf5.png)
 그리고 MLP, GMP만 사용하는것보다 같이 사용하는 NeuMF가 더 좋다는 것을 확인할 수 있었다.

### 4.3 Log Loss with Negative Sampling (RQ2)

![Untitled](/images/2021/Rec_sys/ncf/ncf6.png)
### 4.4 Is Deep Learning Helpful? (RQ3)

![Untitled](/images/2021/Rec_sys/ncf/ncf7.png)
