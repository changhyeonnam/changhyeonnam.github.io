...
4. Implicit feedback으로 데이터를 변환하기 위해 rating(MovieLens Dataset), #pinned(Pinterest Dataset)의 값을 one-hot encoding으로 변환하였다. 이때, 0을 negative instance로 보게되는데, dataset의 대부분이 0으로 채워져있는데 sparsity problem이 생기게 됩니다. 이를 위해서 네개의 negative instance 당 하나의 positive instance을 하나의 sample로 만들어 학습시켰다. 
