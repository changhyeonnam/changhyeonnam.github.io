...
---
```
아직 class4여서 5밀고 있습니다. 골드도 벅찬 나의 실력... 올해안에 500문제 푸는게 목표입니다.
```
...
평면 상 점이 N개 주어지고, 두점을 이어서 벡터를 만들 때, 벡터의 합의 길이의 최소값을 구하는 문제이다. 점이 N개가 주어 진다면, 벡터의 개수는 총 N/2 입니다. N=2k이라 하면 (N이 홀수 일때는 N-1=2k)라 하면,  k개의 점은 더해지고, k개의 점의 좌표는 빼집니다. (벡터는 한 점에서 다른 점을 빼는 것이기 때문에).
...
`multiset` 에 가방 무게를 담고(무게가 중복가능하니),보석의 무게와 가격을 우선순위 큐에 넣고, 보석을 가격 대해 내림 차 순으로 정렬을 했습니다. 가장 비싼 보석 부터 `lower_bound`를 이용하여 담을 수 있는 가장 작은 가방에 담아주었습니다.
...
N개의 물건과 최대 C 만큼의 무게를 넣을 수 있는 가방 하나가 주어질때, N개의 물건을 가방에 넣는 방법의 수를 구하는 문제이다. 모든 부분 집합 중 그 합이 C와 같거나 작은 부분집합의 개수를 세면 되는 문제이다. N≤40이므로 $O(2^n)$은 시간초과가 나지만 N/2≤20 은 O(2^n)을 해도 시간 초과가 나지 않ㅅ다.  절반을 나눠서 부분집합의 합을 구한 뒤, 두 경우를 모두 고려하면 됩니다.
...
N개의 정수로 이루어진 수열이 주어지고, 부분 수열의 합이 S가 되는 경우의 수를 구하는 문제였습니다.(1≤N≤40, |S|≤ 1000000, |주어지는 정수|≤100000). $O(2^N)$은 시간초과가 납니다.
...
N개의 200,000,000 이하의 자연수로 이루어진 집합 U이 주어집니다. x+y+z=k 를 만족하는 원소 x,y,z,k 가  있을때, 최대 k를 찾는 문제 입니다.  x,y,z,k는 중복될 수 있습니다.
