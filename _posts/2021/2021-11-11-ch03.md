...
ping 엔드포인트가 제대로 돌아가는지 실제로 api에 접속해서 테스트해보자. ping 엔드포인트에 접속하기 위해서 ping 엔드포인트에 HTTP 요청을 보내면 된다. hhtpie 툴을 이용하여 http 요청을 보낼것이다. 다음 코드를 실행하면 ping 엔드포인트에 접속하여 get 요청을 보낼 수 있다.

```python
http -v GET http://127.0.0.1:5000/ping
```

`-v` : verbose 옵션에 해당 http 요청과 응답에 관한 추가적인 정보를 출력한다. 다음과 같은 정보가 출력이 된다.
