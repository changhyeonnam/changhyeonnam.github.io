...
파이썬에서는 [클로저는](https://www.scaler.com/topics/python/python-closure/) 자신을 둘러싼 scope의 상태값을 기억하는 함수입니다. 상태값을 기억한다는 의미는 함수가 새로 실행되도 자신을 둘러싼 scope에 대한 캐시가 유지된다는 의미입니다. 어떤 함수가 클로저이기 위해서는 다음 세가지 조건을 만족해야 합니다.
