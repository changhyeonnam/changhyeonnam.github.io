...
> wordvector 자체를 학습시키는 것의 장단점과 그 해결책과 back propagation할때 실제 그 내부에서 어떤일이 일어나는지 배웠다. modern framwork인 pytorch,tensorflow 등에는 Automic  Differentiation이 적용되어 자동으로 계산이 이뤄진다는 것과 마지막으로 grab of bag of miscellaneous stuff라고 교수님이 칭한 학습시 고려해야할 여러가지를 수업했다. 대부분 아는내용 이었지만, symbolic computation을 통한 automatic differentiation은 처음 배웠다.
