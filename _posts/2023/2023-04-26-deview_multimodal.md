...
어떤 이미지와 신발 이라는 카테고리 정보를 인풋으로 넣어주면, 속성 키와 거기에 해당하는 속성 값들을 문장화 해서 학습을 시켰다. 카테고리는 신발 말고도 대분류, 중분류, 소분류 등 다양한 단계로 있기 때문에 학습 시에는 랜덤하게 선택해서 베리에이션을 주었다.
...
구글에서 발표한 CoCa 모델을 타겟으로 했다. Text captioning도 학습을 해서, image-to-text 생성에서도 좋은 성능을 보여주었다. 분류, image-text retreival, captioning 등 다양한 테스크에서 SOTA모델의 성능을 갱신했다.
...
예를들어서 Text를 Transformer의 key, value로 했을때 Image query representation과 Image를 트포의 key, value로 했을 떄의 representation을 근사하게 만들 수가 있다면, modality agnositc한 reasoning에 가까워 질 수 있다고 판단을 하였다.

따라서 앞서 XMC objective에 추가로 Text에 대한 image representation과 Image에 대한 text representation의 평균, 즉 modality agnositic한 값과 Image-text에대한 representation 평균에 대해서 Contrastive Learning Loss를 만들었다.

> XMC objective = modality agnositic한 값
Image-text에대한 representation을 겹랍하여 최종 Contrastiv Learning Loss를 만들었다는 의미.
>
...
![Untitled 49](You are uploading too fast. Please wait 56 more minutes.)
